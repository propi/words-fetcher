...
object Pattern {
  ...
  /**
    ...
    */
  case class ExactPattern private(letters: IndexedSeq[PatternItem]) extends Pattern

  object ExactPattern {
    def apply(letters: IndexedSeq[PatternItem]): ExactPattern = letters.headOption match {
      case Some(PatternItem.Character(x)) => new ExactPattern(letters) with HasLength with HasFirstLetter {
        var length: Int = letters.size
        var firstLetter: Char = x
      }
      case _ => new ExactPattern(letters) with HasLength {
        var length: Int = letters.size
      }
    }
  }
